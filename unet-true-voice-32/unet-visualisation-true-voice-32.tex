
\documentclass[border=8pt, multi, tikz]{standalone} 
\usepackage{import}
\subimport{../layers/}{init}
\usetikzlibrary{positioning}
\usetikzlibrary{3d} %for including external image 

\def\ConvColor{rgb:yellow,5;red,2.5;white,5}
\def\ConvReluColor{rgb:yellow,5;red,5;white,5}
\def\PoolColor{rgb:red,1;black,0.3}
\def\UnpoolColor{rgb:blue,2;green,1;black,0.3}
\def\FcColor{rgb:blue,5;red,2.5;white,5}
\def\FcReluColor{rgb:blue,5;red,5;white,4}
\def\SoftmaxColor{rgb:magenta,5;black,7}   
\def\SumColor{rgb:blue,5;green,15}

\newcommand{\copymidarrow}{\tikz \draw[-Stealth,line width=0.8mm,draw={rgb:blue,4;red,1;green,1;black,3}] (-0.3,0) -- ++(0.3,0);}

\begin{document}
\begin{tikzpicture}
\tikzstyle{connection}=[ultra thick,every node/.style={sloped,allow upside down},draw=\edgecolor,opacity=0.7]
\tikzstyle{copyconnection}=[ultra thick,every node/.style={sloped,allow upside down},draw={rgb:blue,4;red,1;green,1;black,3},opacity=0.7]

\node[canvas is zy plane at x=0] (temp) at (-3,0,0) {\includegraphics[width=8cm,height=8cm]{../examples/noisy.png}};

\pic[shift={(0,0,0)}] at (0,0,0) 
    {Box={
        name=ccr_b1a,
        caption= ,
        xlabel={{32, }},
        zlabel=256,
        fill=\ConvColor,
        height=40,
        width=2,
        depth=0
        }
    };

\pic[shift={(0.5, 0, 0)}] at (ccr_b1a-east) 
    {Box={
        name=ccr_b1b,
        caption= ,
        xlabel={{32, }},
        zlabel=256,
        fill=\ConvColor,
        height=40,
        width=2,
        depth=0
        }
    };

\draw [connection]  (ccr_b1a-east)    -- node {\midarrow} (ccr_b1b-west);

\pic[shift={ (0, -5, 0) }] at (ccr_b1b-southwest) 
    {Box={
        name=pool_b1,
        caption= ,
        fill=\PoolColor,
        opacity=0.5,
        height=20,
        width=2,
        depth=0
        }
    };

\draw [connection]  (ccr_b1b-south)    -- node {\midarrow} (pool_b1-north);

\pic[shift={(0.5, 0, 0)}] at (pool_b1-east) 
    {Box={
        name=ccr_b2a,
        caption= ,
        xlabel={{64, }},
        zlabel=,
        fill=\ConvColor,
        height=20,
        width=4,
        depth=0
        }
    };

\draw [connection]  (pool_b1-east)    -- node {\midarrow} (ccr_b2a-west);

\pic[shift={(0.5, 0, 0)}] at (ccr_b2a-east) 
    {Box={
        name=ccr_b2,
        caption= ,
        xlabel={{64, }},
        zlabel=128,
        fill=\ConvColor,
        height=20,
        width=4,
        depth=0
        }
    };

\draw [connection]  (ccr_b2a-east)    -- node {\midarrow} (ccr_b2-west);

\pic[shift={ (0,-4.5,0) }] at (ccr_b2-east) 
    {Box={
        name=pool_b2,
        caption= ,
        fill=\PoolColor,
        opacity=0.5,
        height=10,
        width=4,
        depth=10
        }
    };

\pic[shift={ (0,0,0) }] at (pool_b2-east) 
    {RightBandedBox={
        name=ccr_b3,
        caption= ,
        xlabel={{ 128, 128 }},
        zlabel=64,
        fill=\ConvColor,
        bandfill=\ConvReluColor,
        height=10,
        width={ 12 , 12 },
        depth=10
        }
    };

\draw [connection]  (ccr_b2-east)    -- node {\midarrow} (ccr_b3-west);

\pic[shift={ (1,0,0) }] at (ccr_b3-east) 
    {Box={
        name=pool_b3,
        caption= ,
        fill=\PoolColor,
        opacity=0.5,
        height=5,
        width=1,
        depth=5
        }
    };

\draw [connection]  (ccr_b3-east)    -- node {\midarrow} (pool_b3-west);

\pic[shift={ (0,0,0) }] at (pool_b3-east) 
    {RightBandedBox={
        name=ccr_b4,
        caption= ,
        xlabel={{ 256, 256 }},
        zlabel=32,
        fill=\ConvColor,
        bandfill=\ConvReluColor,
        height=5,
        width={ 18 , 18 },
        depth=5
        }
    };

\pic[shift={ (1,0,0) }] at (ccr_b4-east) 
    {Box={
        name=unpool_b7,
        caption= ,
        fill=\UnpoolColor,
        opacity=0.5,
        height=10,
        width=1,
        depth=10
        }
    };

\pic[shift={ (0,0,0) }] at (unpool_b7-east) 
    {RightBandedBox={
        name=ccr_res_b7,
        caption= ,
        xlabel={{ 128, }},
        zlabel=64,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=10,
        width=6,
        depth=10
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_b7-east) 
    {Box={
        name=ccr_b7,
        caption= ,
        xlabel={{128, }},
        zlabel=64,
        fill=\ConvColor,
        height=10,
        width=6,
        depth=10
        }
    };

\pic[shift={ (0,0,0) }] at (ccr_b7-east) 
    {RightBandedBox={
        name=ccr_res_c_b7,
        caption= ,
        xlabel={{ 128, }},
        zlabel=64,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=10,
        width=6,
        depth=10
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_c_b7-east) 
    {Box={
        name=end_b7,
        caption= ,
        xlabel={{128, }},
        zlabel=64,
        fill=\ConvColor,
        height=10,
        width=6,
        depth=10
        }
    };

\draw [connection]  (ccr_b4-east)    -- node {\midarrow} (unpool_b7-west);

\path (ccr_b3-southeast) -- (ccr_b3-northeast) coordinate[pos=1.25] (ccr_b3-top) ;
\path (ccr_res_b7-south)  -- (ccr_res_b7-north)  coordinate[pos=1.25] (ccr_res_b7-top) ;
\draw [copyconnection]  (ccr_b3-northeast)  
-- node {\copymidarrow}(ccr_b3-top)
-- node {\copymidarrow}(ccr_res_b7-top)
-- node {\copymidarrow} (ccr_res_b7-north);

\pic[shift={ (0,4.5,0) }] at (end_b7-east) 
    {Box={
        name=unpool_b8,
        caption= ,
        fill=\UnpoolColor,
        opacity=0.5,
        height=20,
        width=1,
        depth=20
        }
    };

\pic[shift={ (0,0,0) }] at (unpool_b8-east) 
    {RightBandedBox={
        name=ccr_res_b8,
        caption= ,
        xlabel={{ 64, }},
        zlabel=128,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=20,
        width=5,
        depth=20
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_b8-east) 
    {Box={
        name=ccr_b8,
        caption= ,
        xlabel={{64, }},
        zlabel=128,
        fill=\ConvColor,
        height=20,
        width=5,
        depth=20
        }
    };

\pic[shift={ (0,0,0) }] at (ccr_b8-east) 
    {RightBandedBox={
        name=ccr_res_c_b8,
        caption= ,
        xlabel={{ 64, }},
        zlabel=128,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=20,
        width=5,
        depth=20
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_c_b8-east) 
    {Box={
        name=end_b8,
        caption= ,
        xlabel={{64, }},
        zlabel=128,
        fill=\ConvColor,
        height=20,
        width=5,
        depth=20
        }
    };

\draw [connection]  (end_b7-east)    -- node {\midarrow} (unpool_b8-west);

\path (ccr_b2-southeast) -- (ccr_b2-northeast) coordinate[pos=1.25] (ccr_b2-top) ;
\path (ccr_res_b8-south)  -- (ccr_res_b8-north)  coordinate[pos=1.25] (ccr_res_b8-top) ;
\draw [copyconnection]  (ccr_b2-northeast)  
-- node {\copymidarrow}(ccr_b2-top)
-- node {\copymidarrow}(ccr_res_b8-top)
-- node {\copymidarrow} (ccr_res_b8-north);

\pic[shift={ (0,8,0) }] at (end_b8-east) 
    {Box={
        name=unpool_b9,
        caption= ,
        fill=\UnpoolColor,
        opacity=0.5,
        height=40,
        width=1,
        depth=40
        }
    };

\pic[shift={ (0,0,0) }] at (unpool_b9-east) 
    {RightBandedBox={
        name=ccr_res_b9,
        caption= ,
        xlabel={{ 32, }},
        zlabel=256,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=40,
        width=3,
        depth=40
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_b9-east) 
    {Box={
        name=ccr_b9,
        caption= ,
        xlabel={{32, }},
        zlabel=256,
        fill=\ConvColor,
        height=40,
        width=3,
        depth=40
        }
    };

\pic[shift={ (0,0,0) }] at (ccr_b9-east) 
    {RightBandedBox={
        name=ccr_res_c_b9,
        caption= ,
        xlabel={{ 32, }},
        zlabel=256,
        fill={rgb:white,1;black,3},
        bandfill={rgb:white,1;black,2},
        opacity=0.5,
        height=40,
        width=3,
        depth=40
        }
    };

\pic[shift={(0,0,0)}] at (ccr_res_c_b9-east) 
    {Box={
        name=end_b9,
        caption= ,
        xlabel={{32, }},
        zlabel=256,
        fill=\ConvColor,
        height=40,
        width=3,
        depth=40
        }
    };

\draw [connection]  (end_b8-east)    -- node {\midarrow} (unpool_b9-west);

\path (ccr_b1b-southeast) -- (ccr_b1b-northeast) coordinate[pos=1.25] (ccr_b1b-top) ;
\path (ccr_res_b9-south)  -- (ccr_res_b9-north)  coordinate[pos=1.25] (ccr_res_b9-top) ;
\draw [copyconnection]  (ccr_b1b-northeast)  
-- node {\copymidarrow}(ccr_b1b-top)
-- node {\copymidarrow}(ccr_res_b9-top)
-- node {\copymidarrow} (ccr_res_b9-north);

\pic[shift={(0.75,0,0)}] at (end_b9-east) 
    {Box={
        name=soft1,
        caption= ,
        zlabel=256,
        fill=\SoftmaxColor,
        height=40,
        width=1,
        depth=40
        }
    };

\draw [connection]  (end_b9-east)    -- node {\midarrow} (soft1-west);

\end{tikzpicture}
\end{document}
